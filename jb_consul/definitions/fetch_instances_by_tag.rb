...
#    if node.has_key?("ec2")
    ruby_block "get instances " + params[:tag_value] do
        ...
        node['jb_consul']['instances'][params[:tag_value]] = JSON.parse(`aws --region #{region} --profile default ec2 describe-instances --filters 'Name=tag:#{params[:tag_name]},Values=#{params[:tag_value]}'` || {})
    end
#    end
end
