...
#    if node.has_key?("ec2")
        check_consul_bootstrap = "aws --region #{region} --profile ~/.aws/config ec2 describe-instances --filters 'Name=tag:#{params[:tag_name]},Values=#{params[:tag_value]}' > #{stack_file_path}#{stack_file}"
